...
[![views](https://request-tracker.femrek.me/api/subscriptions/request/badge/36aadabd-2fab-49fc-bc63-59a4ff5ec354?label=View%20Count)](https://github.com/femrek)
...
Create `.env` file. (lines start with # are optional)
```env
GITHUB_CLIENT_ID=
...
#### b. For Native Run (only spring app is native)
...
Increases the counter of the subscription by one and responses the new subscription.

```http
  GET /no-request/{uuid}
```

Responses the subscription without increasing the counter.
...
```http
  GET /request/badge/{uuid}
```

Increases the counter of the subscription by one and responses shield image as svg.



```http
  GET /no-request/badge/{uuid}
```

Responses the shield image as svg without increasing the counter.
...
- [Shield](https://github/badges/shields)
